...
- AWS本番環境、本番DB、本番利用者データを使う負荷試験
- 20 VUを超える容量限界、Spike、長時間Soak試験
- AWS、Terraform、CDリソースの実装
...
- 同一Mac内の値には実ネットワーク、ALB、ECS、RDSの遅延が含まれないため、本番性能の保証には使わない。ロードマップ16の専用AWS環境で同じ条件を再測定し、今回値をローカル基準として比較する。
