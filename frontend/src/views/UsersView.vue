...
watch(dialogMode, async (mode) => {
  if (mode) {
    ...
    dialog.value?.querySelector<HTMLElement>('input, button, select')?.focus();
...
function openEdit(user: ManagedUser, event: Event): void {
  errorMessage.value = '';
  dialogTrigger = event.currentTarget as HTMLElement;
  ...
  <section>
    <div class="mb-7 flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        <h1 class="text-3xl font-black tracking-tight">ユーザー管理</h1>
      ...
      >
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          ...
          <button
            ...
            class="min-h-11 rounded-xl border px-3 font-bold"
            type="button"
            @click="openAction(user, 'status', $event)"
          >
          ...
          <button
            class="min-h-11 rounded-xl border px-3 font-bold"
            type="button"
            @click="openAction(user, 'reissue', $event)"
          >
            仮パスワード再発行
          </button>
        </div>
    ...
    <div
      ...
      class="hidden overflow-x-auto rounded-2xl border border-[#cfdbd5] bg-white md:block"
    >
